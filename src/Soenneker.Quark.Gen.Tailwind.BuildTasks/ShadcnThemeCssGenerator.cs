...
internal static class ShadcnThemeCssGenerator
{
    ...
    public static string Generate(ShadcnThemeOptions options)
    {
        ...
        var builder = new PooledStringBuilder(4096);
        try
        {
            ...
            AppendScheme(ref builder, ":root", light);
            builder.AppendLine();
            AppendScheme(ref builder, ".dark", dark);
            builder.AppendLine();
            AppendInlineTheme(ref builder, inline);
    ...
    private static void AppendScheme(ref PooledStringBuilder builder, string selector, Dictionary<string, string> values)
    {
        builder.AppendLine(selector);
        builder.AppendLine("{");
        AppendVariables(ref builder, values, _schemeVariableOrder);
        builder.AppendLine("}");
    }

    private static void AppendInlineTheme(ref PooledStringBuilder builder, Dictionary<string, string> values)
    {
        builder.AppendLine("@theme inline");
        builder.AppendLine("{");
        AppendVariables(ref builder, values, _inlineVariableOrder);
        builder.AppendLine("}");
    }

    private static void AppendVariables(ref PooledStringBuilder builder, Dictionary<string, string> values, IReadOnlyCollection<string> orderedNames)
    {
